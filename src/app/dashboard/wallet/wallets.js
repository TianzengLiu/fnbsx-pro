...
export class DepositDetails extends React.Component {
	...
	componentDidMount() {
		...
		var r = this;
		Parse.Cloud.run('getRatesandtotalFnb', {
			"id":Parse.User.current().id
		}, {
		success: function(results) {
			r.setState({"totalFnb":results.totalFnb});
		},error:function(err){
			if (err.code == 209) {
	...
	handleDeposit() {
		...
		var fnbAmount = (parseFloat(this.state.amount) * totalPerDollar).toFixed(5);
			...
			<div>
			{
				...
				<div className="DepositDetailsWrapper">
					...
					<div className="porfolioTable">
						<ul>
							<li>
								<div className="logoP fnb" style={{"background-color":"rgb(77, 138, 255)","color":"white"}}><img style={{"width":"8px"}} src={LogoSW} /></div>
								<div className="porfolioDetails">
									<span>FNB</span>
									<span>Available Balance: {this.state.totalFnb} FNB</span>
								</div>
							</li>
						</ul>
					</div>
...
export class WithdrawalDetails extends React.Component {
	...
	handleInputAmount(e) {
		...
		var maxAmount = parseFloat(this.props.totalFnbBalance);
		if (e.target.value != "") {
			...
			query.first({
			  success: function(object) {
			  	if (object != undefined) {
					...
					$(r.refs.userNotFoundToggle).html('1 FNB Minimum')
					$(r.refs.userNotFoundToggle1).html('1 FNB Minimum')
			  	} else {
			...
			<div className="WithdrawalDetailsWrapper">
					...
					<ul>
						<li>
							...
							<div className="porfolioDetails">
								...
								<span>Available Balance:{this.props.totalFnbBalance}</span>
							</div>
						</li>
					</ul>
				</div>			
				<div className="amountWrapperDetails">
					<div className="amountSendwrapper" style={{"width":"100%","paddingTop":"0"}}>
						<div className="amountToSend">
							<input value={this.state.amount} onChange={this.handleInputAmount} type="tel" className="amountInput"  max={this.props.totalFnbBalance} maxLength="9" placeholder="0.00" />
							<span className="amountSelect" style={{"paddingTop":"18px","cursor":"pointer"}} onClick={(e) => this.setState({"amount":this.props.totalFnbBalance})}>Max</span>
							<br style={{"clear":"both"}} />
						</div>
					</div>
				</div>
