...
class Slider2 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider2_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider2_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider2_forward').hidden = true;
      else document.querySelector('.slider2_forward').hidden = false;

      document.querySelector('.slider2_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider2_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider2').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider2_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider2_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider2_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider2_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider2_content'));
  ...
  render() {
    return (
      <div className="slider2">
        <section className="slider2_title">
          <h3>Film Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider2_icon slider2_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider2_icon slider2_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider2_overflow">
          <section className="slider2_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider2_page">
                <ul className="slider2_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider2_item">
                      <a href="#">
                        <img src={item.url} className="slider2_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider2_content">
                          <p className="slider2_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider2_content_title">
                            {item.projectTitle}
                          </p>
                          <a className="slider2_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          <div className="slider2_progress">
                            <div
                              className="slider2_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>
                          <p className="slider2_status">
                            <span className="slider2_status_raised">
