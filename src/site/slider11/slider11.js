...
class Slider11 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider11_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider11_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider11_forward').hidden = true;
      else document.querySelector('.slider11_forward').hidden = false;

      document.querySelector('.slider11_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider11_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider11').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider11_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider11_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider11_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider11_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider11_content'));
  ...
  render() {
    return (
      <div className="slider11">
        <section className="slider11_title">
          <h3>Video Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider11_icon slider11_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider11_icon slider11_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider11_overflow">
          <section className="slider11_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider11_page">
                <ul className="slider11_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider11_item">
                      <a href="#">
                        <img src={item.url} className="slider11_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider11_content">
                          <p className="slider11_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider11_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider11_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider11_progress">
                            <div
                              className="slider11_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider11_status">
                            <span className="slider11_status_raised">
