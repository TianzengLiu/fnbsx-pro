...
class Slider12 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider12_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider12_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider12_forward').hidden = true;
      else document.querySelector('.slider12_forward').hidden = false;

      document.querySelector('.slider12_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider12_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider12').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider12_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider12_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider12_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider12_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider12_content'));
  ...
  render() {
    return (
      <div className="slider12">
        <section className="slider12_title">
          <h3>Magazine Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider12_icon slider12_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider12_icon slider12_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider12_overflow">
          <section className="slider12_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider12_page">
                <ul className="slider12_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider12_item">
                      <a href="#">
                        <img src={item.url} className="slider12_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider12_content">
                          <p className="slider12_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider12_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider12_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider12_progress">
                            <div
                              className="slider12_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider12_status">
                            <span className="slider12_status_raised">
