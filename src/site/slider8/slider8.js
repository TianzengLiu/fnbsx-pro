...
class Slider8 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider8_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider8_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider8_forward').hidden = true;
      else document.querySelector('.slider8_forward').hidden = false;

      document.querySelector('.slider8_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider8_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider8').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider8_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider8_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider8_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider8_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider8_content'));
  ...
  render() {
    return (
      <div className="slider8">
        <section className="slider8_title">
          <h3>Newspaper Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider8_icon slider8_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider8_icon slider8_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider8_overflow">
          <section className="slider8_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider8_page">
                <ul className="slider8_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider8_item">
                      <a href="#">
                        <img src={item.url} className="slider8_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider8_content">
                          <p className="slider8_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider8_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider8_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider8_progress">
                            <div
                              className="slider8_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider8_status">
                            <span className="slider8_status_raised">
