...
class Slider4 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider4_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider4_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider4_forward').hidden = true;
      else document.querySelector('.slider4_forward').hidden = false;

      document.querySelector('.slider4_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider4_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider4').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider4_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider4_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider4_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider4_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider4_content'));
  ...
  render() {
    return (
      <div className="slider4">
        <section className="slider4_title">
          <h3>Games Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider4_icon slider4_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider4_icon slider4_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider4_overflow">
          <section className="slider4_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider4_page">
                <ul className="slider4_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider4_item">
                      <a href="#">
                        <img src={item.url} className="slider4_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider4_content">
                          <p className="slider4_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider4_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider4_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider4_progress">
                            <div
                              className="slider4_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider4_status">
                            <span className="slider4_status_raised">
