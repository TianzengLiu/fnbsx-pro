...
class Slider14 extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider14_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider14_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider14_forward').hidden = true;
      else document.querySelector('.slider14_forward').hidden = false;

      document.querySelector('.slider14_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider14_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider14').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider14_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider14_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider14_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider14_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider14_content'));
  ...
  render() {
    return (
      <div className="slider14">
        <section className="slider14_title">
          <h3>App Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider14_icon slider14_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider14_icon slider14_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider14_overflow">
          <section className="slider14_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider14_page">
                <ul className="slider14_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider14_item">
                      <a href="#">
                        <img src={item.url} className="slider14_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider14_content">
                          <p className="slider14_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider14_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider14_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider14_progress">
                            <div
                              className="slider14_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider14_status">
                            <span className="slider14_status_raised">
