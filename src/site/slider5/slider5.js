...
class Slider5 extends React.Component {
  ...
  componentDidMount() {
    if (this.pages.length === 1)
      document.querySelector('.slider5_forward').hidden = true;
  ...
  resize() {
    ...
    function reset() {
      document.querySelector('.slider5_backward').hidden = true;

      if (this.length === 1)
        document.querySelector('.slider5_forward').hidden = true;
      else document.querySelector('.slider5_forward').hidden = false;

      document.querySelector('.slider5_items').style.cssText =
  ...
  backward(e) {
    this.currenPage -= 1;
    document.querySelector('.slider5_forward').hidden = false;

    if (this.currenPage === this.firstPage) e.target.hidden = true;

    this.offset += document.querySelector('.slider5').offsetWidth;
  ...
  forward(e) {
    this.currenPage += 1;
    document.querySelector('.slider5_backward').hidden = false;
  ...
  setPageWidth() {
    document.querySelector('.slider5_items').style.width = `${this.length}00%`;

    Array.prototype.forEach.call(document.querySelectorAll('.slider5_page'),
      page => page.style.width = `${this.pageLength}%`);

    Array.prototype.forEach.call(document.querySelectorAll('.slider5_content'),
      item => {
        item.style.whiteSpace = 'normal';

        const itemMetrics = getComputedStyle(
          document.querySelector('.slider5_content'));
  ...
  render() {
    return (
      <div className="slider5">
        <section className="slider5_title">
          <h3>TV/Webseries Auction</h3>
        </section>
        <section>
          <div
            hidden
            className="slider5_icon slider5_backward"
            onClick={e => this.backward(e)}>
          </div>
          <div
            className="slider5_icon slider5_forward"
            onClick={e => this.forward(e)}>
          </div>
        </section>
        <section className="slider5_overflow">
          <section className="slider5_items">
            {this.state.pages.map((page, i) => (
              <div key={i} className="slider5_page">
                <ul className="slider5_list">
                  {page.map((item, i) => (
                    <li key={i} className="slider5_item">
                      <a href="#">
                        <img src={item.url} className="slider5_picture"/>
                      </a>
                      {item.creatorName &&
                        <section className="slider5_content">
                          <p className="slider5_content_creator">
                            {item.creatorName}
                            <span>{item.projectFormat}</span>
                          </p>
                          <p className="slider5_content_title">
                            {item.projectTitle}
                          </p>
                         {/*} <a className="slider5_content_description" href="#">
                            {item.projectDescription}
                      </a>*/}
                          {/*<div className="slider5_progress">
                            <div
                              className="slider5_progress_bar" 
                              style={{ width: `${item.progress}%` }}>
                            </div>
                      </div>*/}
                          {/*<p className="slider5_status">
                            <span className="slider5_status_raised">
